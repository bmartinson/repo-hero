...
- **Flexible time scoping** — filter by 1W / 2W / 3W / 1M / 3M / 6M / 1Y / All directly in the dashboard
...
- **Positive outlier detection** — users performing > 1.5σ above the mean on any metric are flagged with a 🔥 badge and z-score tooltip
...
1. **Gather** — For each week in the date range, queries the GitHub API for pull requests, reviews, and pending commits, and runs `git log` locally for commit counts, LOC, and files touched. Results are saved as `.results_history/YYYY-MM-DD_YYYY-MM-DD.json`. Weeks that already have result files are skipped (idempotent).

2. **Enrich** — Two-pass process: first learns each user's historical commits-per-PR ratio from periods where real PR data exists, then fills in `predictedPullRequests` for periods where PRs are zero.
...
For each metric, the dashboard computes the mean and standard deviation across all active users. Any user exceeding **mean + 1.5σ** is flagged as a positive outlier with a 🔥 badge showing their z-score.

---

## Dashboard

The dashboard is a self-contained HTML file with a dark, console-style theme inspired by NASA mission control interfaces. It includes:
...
- **User profiles** — Click any user card to see their full history and per-metric breakdowns
- **Time scope filter** — 1W / 2W / 3W / 1M / 3M / 6M / 1Y / All buttons with smart x-axis labels (daily for weeks, monthly for longer ranges, yearly for multi-year)
- **Outlier badges** — 🔥 emoji on tiles where a user is a statistical standout
...
├── assets/
│   └── logo.svg              Git-branching logo (used in dashboard header + favicon)
